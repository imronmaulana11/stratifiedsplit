...
base_folder = r'D:/KULIAH/SEMESTER 7/SKRIPSI/DATASET/New dataset/DATASET BARU'  # Adjust to your folder path
...
# Function to determine the label from files in the labels folder
def get_label_from_file(label_file):
    with open(label_file, 'r') as f:
        lines = f.readlines()
        # Extract the first label from each line (YOLO format: class x y w h)
        labels = [int(line.strip().split()[0]) for line in lines if line.strip()]
        # If label 1 (accident) exists, consider it an accident
        if 1 in labels:
            return 1
        # If label 2 (weapon) exists, consider it a weapon
        elif 2 in labels:
            return 2
        # If only label 0 exists, not an accident
        else:
            return 0

# Function to get all files and their labels with improved matching
def collect_files_and_labels():
    image_files = []
    labels = []
    
    # Find all image files with jpg and png formats
    ...
    for img_file in all_image_files:
        ...
        # Try different possible label file name patterns
        possible_keys = [
            img_name_without_ext,                  # Standard format
            os.path.basename(img_file),            # Full filename with extension
            img_name_without_ext.split('.rf.')[0]  # Name before .rf.
        ]
        
        # For augmented files that contain 'rf', try to find original file pattern
        ...
        if not found_label:
            # Try to find a partial match (especially useful for augmented images)
            found_partial = False
...
# Function to create output folders if they don't exist
...
def move_files(image_files, labels, split_indices, split_name):
    ...
    for idx in split_indices:
        ...
        # Get label file path
        # Find matching label file in labels folder
